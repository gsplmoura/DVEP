...
ggplot(pha_redcap, aes(x = as.factor(visit), y = phase_angle, group = record_id)) +
  geom_line(alpha = 0.5, color = "blue") +   # Lines for each individual
  geom_point(alpha = 0.5, color = "blue") +   # Points at each time point
  labs(
    x = "Visit",
    y = "CRP (raw)"
  ) +
...
```{r}
pha_redcap %>% filter(visit == 1 & phase_angle > 9) %>% select(record_id, visit, allocation_group, phase_angle)
```


```{r}
ggplot(pha_redcap, aes(x = as.factor(visit), y = phase_angle)) +
  geom_boxplot(outlier.color = "red", fill = "lightblue", alpha = 0.5) +
  geom_jitter(width = 0.1, alpha = 0.3, color = "blue") + # Adds individual points
  labs(
    title = "Distribution of CRP (raw) Across Visits",
    x = "Visit",
    y = "CRP (raw)"
  ) +
  theme_minimal()
...
Estimar contrastes ortogonais para estimar a diferença entre as médias dos grupos
...
```{r}
pha_1 <- lmer(phase_angle ~ (1 | record_id) + as.factor(visit) + as.factor(allocation_group) + visit * allocation_group, data = pha_redcap)
...
ggplot(data_crp, aes(x = as.factor(visit), y = crp_log, group = record_id)) +
  geom_line(alpha = 0.5, color = "blue") +   # Lines for each individual
  geom_point(alpha = 0.5, color = "blue") +   # Points at each time point
  labs(
    title = "Longitudinal Plot of CRP (raw) Over Visits",
    x = "Visit",
    y = "CRP (raw)"
  ) +
  theme_minimal()
```
```{r}
data_crp %>% filter(visit == 1 & crp_raw > 35) %>%  select(record_id, visit, allocation_group, crp_raw)
```

--> record_id 71: no biological explanation for elevated crp in visit 1 (59.65 -> 2.05 -> 0.31) | Will be included in per-protocol
--> record_id 38: no apparent cause for crp levels | Completed intervetion but lacks labs from V3

```{r}
ggplot(data_crp, aes(x = as.factor(visit), y = crp_raw)) +
  geom_boxplot(outlier.color = "red", fill = "lightblue", alpha = 0.5) +
  #geom_jitter(width = 0.1, alpha = 0.3, color = "blue") + # Adds individual points
  labs(
    title = "Distribution of CRP (raw) Across Visits",
...
```
```{r}
ggplot(data_crp, aes(x = as.factor(visit), y = crp_log)) +
  geom_boxplot(outlier.color = "red", fill = "lightblue", alpha = 0.5) +
  #geom_jitter(width = 0.1, alpha = 0.3, color = "blue") + # Adds individual points
  labs(
    title = "Distribution of CRP (raw) Across Visits",
...
|-------------|-------------|-------------|-----------------------------------|
| **Duration difference** | -0.275 | 0.0076 | Each SD increase in deviation from planned duration predicts a \~24% reduction in CRP (exp(-0.275) ≈ 0.76). This suggests that **poor adherence is strongly associated with lower inflammation reduction**. |
...
- `glmer()` from **lme4**
- `glmmTMB()` from **glmmTMB** — more flexible and often preferred for Gamma models.
...
| Duration Difference| -0.420   | 0.0005   | Significant. Each 1 SD longer than planned duration leads to a 34% reduction in CRP (exp(-0.42) ≈ 0.66) |
...
1. Linear Mixed Model on ΔCRP (Change Score). Collapse the data to one row per participant and model the change in CRP from baseline to final visit. This often reduces heteroscedasticity since we’re modeling change rather than absolute levels.

2. Quantile Mixed Models. Instead of modeling the mean, quantile models can model medians or other quantiles, which are robust to heteroscedasticity and skewness. Example package: lqmm. Models the median instead of the mean — inherently robust to heteroscedasticity.
