...
compliance <- label_variables(compliance,data_codebook)
compliance <- label_choices(compliance)
...
```{r, results = 'hide', message = FALSE, warning = FALSE}
## 9.9 Adverse events
data_events <- filter_data(,1,"events") |> 
    filter(
...
data_events <- label_variables(data_events, data_codebook)
data_events <- label_choices(data_events, data_codebook) 
```
