...
data_bia_incomplete <- read_csv(
    "data_bia_incomplete.csv",
    col_names = TRUE)


# 3. Remove identifying data
data$record_id <- substr(data$record_id,1,2)
data_bia$File <- substr(data_bia$File,1,2)
data_bia_incomplete$File <- substr(data_bia_incomplete$File,1,2)
...
# 4.2 bia
rename_bia <- setNames(object = colnames(data_bia), data_bia_codebook$variable)

data_bia <- data_bia |>
    rename(!!!rename_bia)

data_bia_incomplete <- data_bia_incomplete |>
    rename(!!!rename_bia)
...
data_bia$record_id <- as.integer(data_bia$record_id)
data_bia_incomplete$record_id <- as.integer(data_bia_incomplete$record_id)

## 5.2 function
convert_col_type <- function(column, type) {
  switch(type,
         n = as.numeric(as.character(column)),
         k = lubridate::ymd(column),
...
setdiff(1:75,data_bia_means$record_id)
3  6  7 14 24 25 28 29 30 35 36 40 43 48 49 55 57 59 60 62 64 70 73 74
```{r}
setdiff(1:75,data_bia_means$record_id)
# 3  6  7 14 24 25 28 29 30 35 36 40 43 48 49 55 57 59 60 62 64 70 73 74
...
## Data tibbles
```{r}
# Variables collected exclusively on eleg and d1 to be replicated to d2 and d3
...
d1d3 <- filter_data(c("V1","V3"),0) |> 
    ...
    select(
        ...
        dass_score_depression:ecap_score,
        mean_bp_mean, 
        handgrip, 
        evs_score, 
        alcohol_dose, alcohol_significant, 
        smoke_history, pack_years, 
        carbs_kcal, protein_kcal, fat_kcal, 
        cycle_phase, drugs_dose_change_yn, drugs_dose_change_notes, 
        intervention_prevention_reason_yn, 
        specify_intervention_prevention_reasons, 
        intervention_delivered_yn, 
        explain_intervention_not_delivered
        )

d1 <- full_join(d0, d1, by = join_by(record_id))
```


view(
    data |> filter(
        !is.na(handgrip_right_1) | !is.na(handgrip_left_1)
    ) |> 
    select(record_id, event_name, handgrip_right_1:handgrip_complete)
)


## Compliance
1. Compute Days Between Visits

```{r}
# intervention_start_date: data$intervention_start_date[data$event_name == "eleg_arm_1" & !is.na(data$intervention_start_date)]
# second visit: data$evaluation_date[data$event_name == "2visit_arm_1" & !is.na(data$evaluation_date)]
# third visit: data$evaluation_date[data$event_name == "3visit_arm_1" & !is.na(data$evaluation_date)]

compliance <- data |> 
    mutate(
        days_to_visit_2 = if_else(
            !is.na(data$evaluation_date[data$event_name == "2visit_arm_1"),
            as.numeric(
                difftime(
                    data$evaluation_date[data$event_name == "2visit_arm_1"], 
                    data$intervention_start_date[data$event_name == "eleg_arm_1"],
                    units = "days"
                )
            ),
            NA_real_
        ),
        days_to_visit_3 = if_else(
            !is.na(data$evaluation_date[data$event_name == "3visit_arm_1"),
            as.numeric(
                difftime(
                    data$evaluation_date[data$event_name == "3visit_arm_1"],
                    data$evaluation_date[data$event_name == "2visit_arm_1"], 
                    units = "days"
                )
            ),
...
compliance <- data |>
    ...
    mutate(
        days_to_visit_2 = if_else(
            event_name == "2visit_arm_1",
            as.numeric(
                difftime(
                    evaluation_date,
                    intervention_start_date[event_name == "eleg_arm_1"],
                    units = "days"
                )
            ),
            NA_real_
        ...
        days_to_visit_3 = if_else(
            event_name == "3visit_arm_1",
            as.numeric(
                difftime(
                    evaluation_date,
                    evaluation_date[event_name == "2visit_arm_1"],
                    units = "days"
                )
            ),
            NA_real_
...
filter_data("eleg",0) |> 
    filter(!is.na(intervention_start_date)) |> 
    select(intervention_start_date)
