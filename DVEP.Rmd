...
## Bioimpedance data
```{r}
# 1. D3 DATA (contains data from first and third visits for participants who completed the intervention)
...
# 2. D1 DATA (data from the first visit for participants who did not complete the intervention)
## Filter lines for which phaseangle is <> ""
...
# 3. MERGE D1/D3 BIA data into a single tibble
data_bia <- bind_rows(
...
## DVEP REDCap Data tibbles
### Variables collected exclusively on eleg and d1 to be replicated to d2 and d3 (`d1_exclusive`)

```{r}
# 1. Variables collected exclusively on eleg and d1 to be replicated to d2 and d3 (`d1_exclusive`)
...
I21_conditions_R |> 
    ...
    mutate(percentage = frequency/75 * 100) |> 
    view()
```
    ...
    mutate(
        hypertension = 
            if_else(
                ...
                1,
                if_else(
                    record_id %in% (I22_drugs_R |> 
                                         filter(str_detect(drugs_sql, "C66869|C29098|C61635|C28836|C29254|C62027|C62027_2")) |> 
                                         pull(record_id)),
                    1,
                    0
                )
...
```{r}
# 2. Non-repeating data common to V1 and V3 (`d1d3`)
...
# 3. Non-repeating data from the second visit (`d2`)
d2 <- filter_data("V2",0) |> 
    mutate(
        visit = case_when(
...
# 4. Bind rows for non-repeating variables from D1/D2/D3
all_data <- bind_rows(
    d1d3,d2
) |> 
    mutate(
