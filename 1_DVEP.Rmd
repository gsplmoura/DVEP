...
# Verifies if label_column exists; throws stop () if not
# Uses trimws () on codebook$variable to avoid mismatch due to leading/trailing spaces
# stop () with informative message if no matching vars found
# Warns if variable has multiple labels (non-unique match)
...
# ------------------
# ### DEPRECATED on April 09th, 2025 ###
# ------------------
# Reasons for deprecation 
# Output Format: The newer version provides better output by pivoting to wide format for grouped summaries, making it more user-friendly.
# Consistent Logic: Both versions use the same approach for calculating the mean and 95% confidence intervals, extracting variable labels, and handling numeric columns.
# Improved Presentation: The wide format in the newer version improves readability for reports and # publications.
# If you need the long format, consider adapting the workflow to the new version or adding an option to toggle between formats.
